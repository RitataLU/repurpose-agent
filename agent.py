...
def save_outputs(rows: list, best: dict):
    ...
    # Write docs/data.js so HTML pages work when opened via file:// locally.
    # Browsers block fetch() on file://, but <script src="data.js"> works fine.
    with open("docs/data.js", "w") as f:
        f.write("// Auto-generated by agent.py — do not edit\n")
        f.write("window._RA_RESULTS = " + json.dumps(rows_out, default=str) + ";\n")
        f.write("window._RA_GENES   = " + json.dumps(genes_out) + ";\n")
        f.write("window._RA_GOLD    = " + json.dumps(gold_out)  + ";\n")
